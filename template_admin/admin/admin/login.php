...
<style type="text/css">
	body {
  background:#6f8489 url({$skin_path}/images/login-bg.jpg) left top no-repeat;background-attachment: fixed; background-size: cover;
  min-height: 100%;
}
...
<input id="btm_sendMobileCode" onclick="sendMobileCode('{url('tool/smscode',false)}',$('#tel'));" type="button" value="{lang('send_cell_phone_verification_code')}" />
...
<input type='text' id="verify"  tabindex="3" onKeyUp="value=value.replace(/[\W]/g,'')" name="verify" />{verify()}
{/if}
